...
import {
  getPetsWithSuccessStories,
  GetPetsWithSuccessStoriesParams,
} from '@/controllers/PetController/getPetsWithSuccessStories'

type Props = PageSearchParamsProps2<GetPetsWithSuccessStoriesParams>

export async function PetList(props: Props) {
  const { searchParams } = props
  const { data: pets, links } = await getPetsWithSuccessStories(searchParams)
